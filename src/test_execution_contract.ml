...
let test_cross_language_source_identity () =
  ...
  in
  expect
    (Document.execution_identity_parts indented_fence = [ ("block", "let x = 1\n") ])
...
let test_project_digest_identity () =
  ...
  in
  let target_a =
    Document.parse ~path:"target.ml.md" "    let result = Dependency.answer + 1\n"
  in
  let target_b =
    Document.parse ~path:"target.ml.md" "    let result = Dependency.answer + 2\n"
  in
  let digest_a =
    Evaluator.project_digest ~documents:[ dependency; target_a ] ~target:target_a
  in
  let digest_b =
    Evaluator.project_digest ~documents:[ dependency; target_b ] ~target:target_b
  in
  let digest_changed_dependency =
    Evaluator.project_digest
      ~documents:[ changed_dependency; target_b ] ~target:target_b
  in
  expect (String.equal digest_a digest_b)
    "editing the target changed the dependency project digest";
  expect (not (String.equal digest_b digest_changed_dependency))
...
let test_execution_identity_width () =
  ...
  let source_map_field name =
    artifact |> Yojson.Safe.Util.member "sourceMaps"
    |> Yojson.Safe.Util.member name |> Yojson.Safe.Util.to_string
  in
  expect (String.length (field "compilerInputsDigest") = 64)
    "compiler input identity is not SHA-256";
  expect (String.length (field "codeRevisionId") = 64)
    "code revision identity is not SHA-256";
  expect (String.length (source_map_field "extractedCodeDigest") = 64)
...
let test_user_runtime_ownership () =
  ...
  expect
    (List.for_all
       (fun trace ->
         List.mem (trace |> member "siteId" |> to_string) ids)
       traces)
...
let test_aliased_pattern_observation () =
  let document =
    Document.parse ~path:"alias.ml.md"
      ...
      \    let value = function Node item as tree -> item, tree | Empty -> 0, Empty\n\
  ...
  let constructors =
    ...
    |> List.filter (fun construct ->
        String.equal construct.Evaluator.construct_semantic_kind
          "constructor"
  ...
  let observed =
    ...
    |> List.map (fun occurrence ->
        occurrence |> Yojson.Safe.Util.member "constructId"
        |> Yojson.Safe.Util.to_string)
  in
  expect
    (List.exists
       (fun construct ->
         List.mem construct.Evaluator.construct_id observed)
       constructors)
  ...
  let alias_occurrences =
    ...
    |> List.filter (fun construct_id ->
        List.exists
          ...
          aliases)
  in
  expect (List.length alias_occurrences = 1)
...
let test_nested_pattern_observations () =
  let document =
    Document.parse ~path:"nested-patterns.ml.md"
      ...
      \    let inspect input =\n\
      \      match input with\n\
      \      | ((A _ as whole), ((Left value | Right value) as side)) when value < 0 -> 0\n\
  ...
  let find_node ~line ~column ~kind =
    ...
    | [] ->
      ...
      in
      fail
        (Printf.sprintf "no %s pattern construct covered %d:%d (available: %s)"
           kind line column available)
  in
  ...
  let guarded_alternative =
    find_node ~line:5 ~column:34 ~kind:"alternative"
  in
  ...
  expect_details guarded_tuple
    ...
    "a tuple pattern did not retain its exact matched value before a false guard";
  ...
  expect_details guarded_alternative [ "Left (2)"; "Right (3)" ]
  ...
  expect_details guarded_side_alias [ "Left (2)"; "Right (3)" ]
  ...
  expect (List.length (returns unguarded_tuple) = 2)
  ...
  expect (returns unmatched_constructor = [])
    "an unmatched constructor emitted a runtime occurrence";
  let range_keys =
    constructs
    |> List.map (fun construct ->
        ( construct.Evaluator.construct_start_byte,
          construct.construct_end_byte ))
  in
  expect
    (List.length range_keys
     = List.length (List.sort_uniq compare range_keys))
...
let test_crlf_source_map () =
  ...
  in
  expect (mapped.map_start_utf16 = 29 && mapped.map_end_utf16 = 30)
    (Printf.sprintf
...
let () =
  ...
  let document =
    Document.parse ~path:"contract.ml.md"
      ...
      \    let rec countdown value = if value = 0 then 0 else countdown (value - 1)\n\
      \    let counted = countdown 2\n\
      \    let local value =\n\
      \      let rec loop current = if current = 0 then 0 else loop (current - 1) in\n\
      \      loop value\n\
  ...
  expect
    (List.length scope_ids = List.length (List.sort_uniq String.compare scope_ids))
  ...
  expect
    (List.for_all
       ...
       constructs)
    "compiler byte ranges, containment, or execution-scope ownership were invalid";
  expect
    (List.for_all
       ...
       (fun scope ->
         ...
         | Some function_construct_id ->
           let children =
             List.filter
               (fun construct ->
                 Option.equal String.equal construct.Evaluator.construct_parent_id
                   (Some function_construct_id))
               constructs
           in
           children <> []
  ...
  let normalized_selector_ids =
    List.map (fun selector -> selector |> member "id" |> to_string)
      normalized_selectors
  in
  let mapped_selector_ids =
    List.map (fun entry -> entry |> member "selectorId" |> to_string)
  ...
  let activations = artifact |> member "execution" |> member "activations" |> to_list in
  expect
    (List.for_all
       (fun activation ->
         match activation |> member "functionConstructId" with
         | `Null -> true
         | `String construct_id ->
           let expected_scope = List.assoc construct_id function_scope_by_construct in
           String.equal (activation |> member "scopeId" |> to_string)
             expected_scope
         | _ -> false)
       activations)
  ...
  expect
    (List.for_all
       (fun selector ->
         ...
         && not
              (String.equal selector.Evaluator.selector_syntax_fingerprint ""))
       selectors)
  ...
  let equals_selectors =
    List.filter
      (fun selector ->
        String.equal selector.Evaluator.selector_role "equals")
      selectors
  in
  ...
  let rec_selectors =
    List.filter
      ...
      selectors
  in
  expect (List.length rec_selectors >= 2)
  ...
  let function_context_selectors =
    List.filter
      ...
      selectors
  in
  expect (function_context_selectors <> [])
  ...
  expect
    (List.for_all
       (fun selector ->
         if not (String.equal selector.Evaluator.selector_role "binder") then true
         else
           match
             List.find_opt
               (fun construct ->
